...
public interface JobRepository extends JpaRepository<Job, Long> {
       ...
       @Query(value = "SELECT * FROM jobs WHERE status = 'APPROVED' AND " +
                     ...
                     "ORDER BY created_at DESC", countQuery = "SELECT count(*) FROM jobs WHERE status = 'APPROVED' AND "
                                   +
                                   "(title ~* :skillRegex OR description ~* :skillRegex OR requirements ~* :skillRegex)", nativeQuery = true)
       ...
       long countByRecruiterIdAndStatus(@Param("recruiterId") Long recruiterId, @Param("status") Job.Status status);
